...
function storeMediaItems(mediaItems) {
    ...
    for (const mi of mediaItems) {
        _mediaItems[mi.id] = [mi.productUrl, mi.baseUrl];
    }
}
...
async function runAsync(checkSharedAlbums) {
    ...
    if (Object.keys(_mediaItems).length) {
        ...
        for (const id in _mediaItems) {
            const url = _mediaItems[id][0],
                  imgSrc = _mediaItems[id][1] + '=w200-h100',
                  tr = document.createElement('tr');

            tr.innerHTML =
                //`<td>${id}<td>` +
                `<td><a href='${url}' target='_blank'><img src='${imgSrc}'></a><td>`;
...
export default [
    {
        name: 'Find out-of-album photos (with thumbnail)',
        scopes: 'https://www.googleapis.com/auth/photoslibrary.readonly',

        async run() {
            try {
                console.log('findOutOfAlbumPhotosWithThumbnail : running');
                const output = await runAsync(false);
                console.log('findOutOfAlbumPhotosWithThumbnail : finished');
                return output;
            }
            catch (err) {
                return err.toString();
    ...
    {
        name: 'Find out-of-album photos (with thumbnail, including "shared" albums)',
        scopes: 'https://www.googleapis.com/auth/photoslibrary.readonly',

        async run() {
            try {
                console.log('findOutOfAlbumPhotosWithThumbnail(w/shared) : running');
                const output = await runAsync(true);
                console.log('findOutOfAlbumPhotosWithThumbnail(w/shared) : finished');
                return output;
            }
            catch (err) {
                return err.toString();
